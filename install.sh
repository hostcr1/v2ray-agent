...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> Please refer to this tutorial for adding methods, https://github.com/hostcr1/v2ray-agent/blob/master/documents/dns_txt.md"
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/hostcr1/v2ray-agent/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://github.com/hostcr1/v2ray-agent/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/hostcr1/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "Author: mack-a"; then
		...
		if [[ "${vasmaType}" == "true" ]]; then
			echoContent green "Shortcut created successfully, execute [vasma] to reopen the script"
		fi
	fi
}
...
menu() {
	...
	echoContent green "Github: https://github.com/hostcr1/v2ray-agent"
